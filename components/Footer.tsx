import React from 'react';
import { COMPANY_INFO } from '../constants';
import { Facebook, Instagram, Linkedin, Home, Wrench, ShoppingBag, MessageSquare, Mail, Phone } from 'lucide-react';

const Footer: React.FC = () => {
  return (
    <footer className="bg-sures-dark text-white py-16 border-t border-white/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
          
          <div className="col-span-1 md:col-span-2">
            {/* Logo PNG original */}
            <img 
              src="/logo-sures.png" 
              alt="Sures Climatización" 
              className="h-16 w-auto mb-6"
            />

            <p className="text-gray-400 max-w-sm">
              {COMPANY_INFO.legalName}
              <br/>
              Líderes en climatización integral en Zona Sur. Compromiso, calidad y tecnología al servicio de su confort.
            </p>
          </div>

          <div>
            <h3 className="font-bold mb-4">Enlaces Rápidos</h3>
            <ul className="space-y-3 text-gray-400">
              <li>
                <a href="#home" className="hover:text-white transition-colors flex items-center gap-2">
                  <Home size={16} />
                  Inicio
                </a>
              </li>
              <li>
                <a href="#services" className="hover:text-white transition-colors flex items-center gap-2">
                  <Wrench size={16} />
                  Servicios
                </a>
              </li>
              <li>
                <a href="#catalog" className="hover:text-white transition-colors flex items-center gap-2">
                  <ShoppingBag size={16} />
                  Catálogo
                </a>
              </li>
              <li>
                <a href="#contact" className="hover:text-white transition-colors flex items-center gap-2">
                  <MessageSquare size={16} />
                  Contacto
                </a>
              </li>
            </ul>
          </div>

          <div>
            <h3 className="font-bold mb-4">Contacto</h3>
            <ul className="space-y-3 text-gray-400">
              <li>
                <a href={`mailto:${COMPANY_INFO.email}`} className="hover:text-white transition-colors flex items-center gap-2">
                  <Mail size={16} className="flex-shrink-0" />
                  <span className="break-all">{COMPANY_INFO.email}</span>
                </a>
              </li>
              <li>
                <a href={`tel:${COMPANY_INFO.phone1.replace(/\s/g, '')}`} className="hover:text-white transition-colors flex items-center gap-2">
                  <Phone size={16} className="flex-shrink-0" />
                  {COMPANY_INFO.phone1}
                </a>
              </li>
              <li>
                <a href={`tel:${COMPANY_INFO.phone2.replace(/\s/g, '')}`} className="hover:text-white transition-colors flex items-center gap-2">
                  <Phone size={16} className="flex-shrink-0" />
                  {COMPANY_INFO.phone2}
                </a>
              </li>
            </ul>
          </div>

          <div>
            <h3 className="font-bold mb-4">Síganos</h3>
            <ul className="space-y-3 text-gray-400">
              <li>
                <a 
                  href={`https://wa.me/${COMPANY_INFO.whatsapp}`} 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="hover:text-white transition-colors flex items-center gap-2"
                >
                  <span className="p-2 bg-white/10 rounded-full">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                      <path d="M17.472 14.382C17.112 14.202 15.344 13.332 15.016 13.212C14.688 13.092 14.448 13.032 14.208 13.392C13.968 13.752 13.28 14.562 13.064 14.802C12.856 15.042 12.632 15.072 12.272 14.892C11.912 14.712 10.752 14.332 9.376 13.102C8.288 12.132 7.552 10.932 7.336 10.562C7.12 10.192 7.312 9.992 7.496 9.812C7.656 9.652 7.848 9.392 8.024 9.192C8.2 8.992 8.256 8.842 8.376 8.602C8.496 8.362 8.432 8.152 8.344 7.972C8.256 7.792 7.552 6.062 7.256 5.362C6.968 4.682 6.672 4.772 6.456 4.772C6.264 4.772 6.04 4.772 5.816 4.772C5.592 4.772 5.232 4.862 4.928 5.192C4.624 5.522 3.768 6.332 3.768 7.982C3.768 9.632 4.944 11.222 5.112 11.442C5.28 11.662 7.424 14.972 10.712 16.392C11.496 16.732 12.104 16.932 12.584 17.082C13.432 17.352 14.208 17.312 14.824 17.222C15.512 17.122 16.936 16.362 17.232 15.522C17.528 14.682 17.528 13.972 17.472 13.882C17.416 13.792 17.176 13.732 16.816 13.552H17.472V14.382ZM12.008 24C16.328 24 20.072 21.682 22.184 18.042C24.296 14.402 24.296 9.882 22.184 6.242C20.072 2.602 16.328 0.282 12.008 0.282C7.688 0.282 3.944 2.602 1.832 6.242C-0.28 9.882 -0.28 14.402 1.832 18.042L0.28 23.722L6.104 22.192C7.848 23.352 9.896 24 12.008 24Z" />
                    </svg>
                  </span>
                  WhatsApp
                </a>
              </li>
              <li>
                <a href="#" className="hover:text-white transition-colors flex items-center gap-2">
                  <span className="p-2 bg-white/10 rounded-full">
                    <Facebook size={16} />
                  </span>
                  Facebook
                </a>
              </li>
              <li>
                <a href="#" className="hover:text-white transition-colors flex items-center gap-2">
                  <span className="p-2 bg-white/10 rounded-full">
                    <Instagram size={16} />
                  </span>
                  Instagram
                </a>
              </li>
              <li>
                <a href="https://www.linkedin.com/company/sures-climatizaci%C3%B3n/" target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors flex items-center gap-2">
                  <span className="p-2 bg-white/10 rounded-full">
                    <Linkedin size={16} />
                  </span>
                  LinkedIn
                </a>
              </li>
            </ul>
          </div>
        </div>
        
        <div className="mt-16 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center text-sm text-gray-500">
          <p>© {new Date().getFullYear()} Sures. Todos los derechos reservados.</p>
          <div className="flex space-x-6 mt-4 md:mt-0">
             <a 
               href="https://www.carrier.com.ar/" 
               target="_blank" 
               rel="noopener noreferrer"
               className="hover:text-white transition-colors"
             >
               Dealer Oficial Carrier
             </a>
             <a 
               href="https://store.midea.com.ar/" 
               target="_blank" 
               rel="noopener noreferrer"
               className="hover:text-white transition-colors"
             >
               Dealer Oficial Midea
             </a>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;